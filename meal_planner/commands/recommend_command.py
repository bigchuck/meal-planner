...
class RecommendCommand(Command, CommandHistoryMixin):
    ...
    overview_help = (
        ...
        "\n"
        "Subcommands:\n"
        "  generate <meal_type> [--method history|exhaustive|ga] [--count N] [--template <name>] [--restart]\n"
        "  status [--verbose]\n"
        "  show [<array>|<view>] [<range>|<id>] [--items] [--gaps] [--excesses] [--nutrients] [--verbose]\n"
        "  filter [--verbose] [--refilter]\n"
        "  score [--verbose] [--rescore]\n"
        "  accept <G-ID|GA-N> [--as <workspace_id>] [--desc <text>]\n"
        "  subset scored --where <expr> --name <view_name>\n"
        "  sort <view_name> --keys \"field:dir,...\"\n"
        "  discard [raw|filtered|scored] | discard view <n>|all\n"
        "  reset [--force]\n"
        "  debugdump <raw|filtered|rejected|scored> <filename>\n"
        "\n"
    ...
    detailed_help = (
        ...
        "\n"
        "PIPELINE FLOW:\n"
        "  1. recommend generate <meal_type>   Generate raw candidates\n"
        "  2. recommend status                 Check pipeline state\n"
        "  3. recommend filter                 Apply pre-score filters\n"
        "  4. recommend score                  Score filtered candidates\n"
        "  5. recommend show scored            View top recommendations\n"
        "  6. recommend accept G3              Accept a candidate to workspace\n"
        "  7. recommend discard                Clean up when done\n"
        "\n"
        "SUBCOMMANDS:\n"
        "\n"
        "  generate <meal_type> [--method <method>] [--count N] [--template <name>] [--restart]\n"
        "    Generate meal candidates for the given meal type.\n"
        "    --method    history | exhaustive | ga  (default: exhaustive)\n"
        "    --count     Number of candidates to generate (not valid with --method ga)\n"
        "    --template  Template name to use for exhaustive generation\n"
        "    --restart   GA only: delete existing GA file and start fresh\n"
        "\n"
        "  status [--verbose]\n"
        "    Show pipeline state: raw / filtered / rejected / scored counts.\n"
        "    --verbose   Include rejection reason breakdown\n"
        "\n"
        "  show [<array>|<view>] [<range>|<id>] [--items] [--gaps] [--excesses] [--nutrients] [--verbose]\n"
        "    Display candidates. Single candidate always shows full detail.\n"
        "    Multiple candidates default to detailed view; add any display flag for compact view.\n"
        "    <array>     raw | filtered | rejected | scored | ga\n"
        "    <view>      A named subset created with 'subset'\n"
        "    <range>     Single position (5), range (1-10), or list (1,3,7)\n"
        "    <id>        Candidate ID (e.g. G3) or GA member (e.g. GA-1)\n"
        ...
        "\n"
        "  filter [--verbose] [--refilter]\n"
        "    Apply pre-score filters to unfiltered candidates.\n"
        "    --refilter  Clear existing filter/score results and reprocess all\n"
        "    --verbose   Show detailed filter output\n"
        "\n"
        "  score [--verbose] [--rescore]\n"
        "    Score filtered candidates and rank them.\n"
        "    --rescore   Clear existing scores and reprocess all filtered candidates\n"
        "    --verbose   Show detailed scoring info\n"
        "\n"
        "  accept <G-ID|GA-N> [--as <workspace_id>] [--desc <text>]\n"
        "    Accept a candidate and promote it to the planning workspace.\n"
        "    --as        Assign a specific workspace ID\n"
        "    --desc      Set a description on the accepted meal\n"
        "\n"
        "  subset scored --where <expr> --name <view_name>\n"
        "    Create a named filtered view of scored candidates.\n"
        "    --where     Filter expression using field names and comparisons\n"
        "                Fields: score, cal, protein, carbs, fat, fiber, gl, sodium,\n"
        "                        potassium, sugar, vita, vitc, iron\n"
        "                Operators: > < >= <= = and or\n"
        "    --name      Name for the view (used with 'show' and 'sort')\n"
        "\n"
        "  sort <view_name> --keys \"field:dir[,field:dir,...]\"\n"
        "    Sort a named view by one or more fields.\n"
        "    dir: asc | a | desc | d\n"
        "\n"
        "  discard [raw|filtered|scored]\n"
        "    Discard candidates from the specified array (or all if omitted).\n"
        "    discard view <n>|all   Remove a specific candidate by position or all\n"
        "\n"
        "  reset [--force]\n"
        "    Clear the entire recommendation session.\n"
        "    --force     Skip confirmation prompt\n"
        "\n"
        "  debugdump <raw|filtered|rejected|scored> <filename>\n"
        "    Write candidate array to a JSON file for inspection.\n"
        "\n"
        "EXAMPLES:\n"
        "  recommend generate lunch --method exhaustive --count 200\n"
        "  recommend generate breakfast --method ga\n"
        "  recommend generate breakfast --method ga --restart\n"
        "  recommend filter --verbose\n"
        "  recommend score\n"
        "  recommend show scored 1-10 --items\n"
        "  recommend show scored --gaps --excesses\n"
        "  recommend subset scored --where \"score>85 and fiber>5\" --name finalists\n"
        "  recommend sort finalists --keys \"score:desc,fiber:desc\"\n"
        "  recommend show finalists --nutrients\n"
        "  recommend accept G3 --desc \"Monday lunch\"\n"
        "  recommend show GA-1 --verbose\n"
        "  recommend discard raw"
    )
    ...
    def _show_all_candidates(
        ...
        if items:
            ...
            for i, candidate in enumerate(display_candidates):
                ...
                if not source_date:
                    source_date = candidate.get("generation_metadata", {}).get("method")
                    if source_date:
                        source_date = "generated by: " + source_date + ","
                    else:
                        source_date = "unknown"
    ...
    def _discard(self, args: List[str]) -> None:
        ...
        if full_reset or raw_count > 0:
            ...
            if 'filtered' in target_arrays:
                ...
                view_count = len(reco_workspace["views"], {})
